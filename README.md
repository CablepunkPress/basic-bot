...
- **Short-term (sliding window):** The most recent messages in verbatim form. The window holds between 20 and 40 messages before a fold compresses the oldest batch. This is what the model sees directly in its context.
...
RAG and summary are coupled by design: if RAG embedding fails, the summary does not fire. This prevents a gap where messages pass the boundary but are never indexed as vectors. Both succeed or neither does. The raw messages are never deleted regardless — they remain in the database and can always be recovered.
